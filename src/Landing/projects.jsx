...
const Beranda = () => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return dataProject.filter(
      (project) => project.relationships?.category?.id === String(selectedCategoryId)
    );
    ...
    <section className="bg-white">
      <div className="mt-24 px-4 md:px-8 lg:px-16 mx-auto animate-fadeSlideUp">
        ...
        <div className="w-full text-center">
          <h1 className="text-xl md:text-4xl font-bold text-black mb-6">My Project</h1>
        </div>

        {/* Search Box */}
        <div className="w-full py-6">
          <div className="relative">
            <input className="w-full bg-white placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-3xl pl-3 pr-28 py-4 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-lg focus:shadow" placeholder="Search Here..." 
             onChange={(e) => handleChange("title", e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleSearch();}}/>
            <button className="absolute mt-1 top-1 right-1 flex items-center rounded-3xl bg-slate-800 py-2 px-8 border border-transparent text-sm text-white shadow-sm hover:shadow focus:bg-slate-700 focus:shadow-none active:bg-slate-700 mr-1" type="button" onClick={handleSearch}>
          ...
          <div className="flex gap-4 w-max">
            {Array.isArray(dataCategory) &&
              dataCategory.map((category) => (
                <button key={category.id} onClick={() => handleProjectCategory(String(category.id))} className="rounded-xl px-4 py-2 bg-orange-300 hover:bg-orange-400 active:bg-orange-500 flex-shrink-0">
                  ...
                  <div className="p-3 bg-purple-300 min-h-[5rem] w-full">
                    <div className="bg-white shadow-xl p-3 rounded-lg grid grid-cols-6">
                      ...
                      <div className="col-span-1 flex justify-end">
                        <button onClick={() => handleDetail(project.id)} className="bg-pink-400 hover:bg-pink-500 shadow-xl rounded-2xl h-[3rem] w-[3rem] flex items-center justify-center">
            ...
            filteredProjects.map((projectCategory) => (
              <div key={projectCategory.id} className="shadow-lg bg-cover bg-center min-h-[15rem] w-full rounded-xl overflow-hidden flex flex-col justify-end relative"
                ...
                <div className="p-3 bg-purple-300 min-h-[5rem] w-full">
                  <div className="bg-white shadow-xl p-3 rounded-lg grid grid-cols-6">
                    <div className="col-span-5">
                      <h2 className="font-bold truncate">{projectCategory.attributes.title}</h2>
                      <p className="text-sm">
                        {projectCategory.attributes.description.split(" ").slice(0, 3).join(" ") ?? ""}...
                      </p>
                    </div>
                    <div className="col-span-1 flex justify-end">
