              ...
              <div className="pb-2 ">
                <label className="relative cursor-pointer block w-full group">
                  {selectedHeroImage || selectedNewImage ? (
                    ...
                    <div>
                      <div>singleImage</div>
                      {/* <img src={image.attributes.url instanceof File ? URL.createObjectURL(image.attributes.url) : image.attributes.url} className="w-16 h-16 object-cover rounded-xl" alt={image.attributes.name} /> */}

                      <img src={projectImage[indexProjectImage].attributes.url instanceof File ? URL.createObjectURL(projectImage[indexProjectImage].attributes.url) : projectImage[indexProjectImage].attributes.url} alt="Single Image" className="w-full max-h-80 object-cover rounded-xl mb-2" />
                    </div>
                  ...
                  {imageFileList[indexIter - 1] && !saveIndex && !isProjectImage ? (
                    <div>
                      <div>imageFileList[indexIter - 1] && !saveIndex && !isProjectImage </div>
                  ...
                  ) : newImageModal && !isProjectImage? (
                    <div>
                      <div>newImageModal && !isProjectImage</div>
                    ...
                    <div className="flex gap-2 whitespace-nowrap min-w-max">
                      ...
                      {projectImage.map((image, index) => (
                        <div key={image.id} className="flex items-center mb-2">
                          <button type="button" onClick={() => { handleProjectImage(String(image.id)); setIndexProjectImage(index); setIsProjectImage(true); }} className="h-fit w-fit rounded-2xl justify-center items-center flex-shrink-0 relative inline-block hover:border-2 active:border-2 border-transparent hover:border-purple-300 active:border-blue-300">
                            <img src={image.attributes.url instanceof File ? URL.createObjectURL(image.attributes.url) : image.attributes.url} className="w-16 h-16 object-cover rounded-xl" alt={image.attributes.name} />
                            {/* <div>{String(image.id)}</div> */}
                          </button>
                        </div>
                      ))}
                      ...
                      {imageFileList.map((img, index) => (
                        <button onClick={() => setSaveIndex((index), setIsProjectImage(false), singleImage(false))} type="button" key={index} className="mb-2">
                          {img instanceof File ? (
                      ...
                      {/* {console.log("indexProjectImage: ", indexProjectImage)}
                      {console.log("ProjectImage: ", projectImage)}
                      {console.log("imageFileList: ", imageFileList)} */}
                      {console.log("imageFileList: ", isProjectImage)}
