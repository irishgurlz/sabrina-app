...
const Table = () => {
  const { inputValues = {}, handleSearch, fetchProjects, handleChange = [] } = useContext(GlobalContext);
  ...
  useEffect(() => {
    if (!token) {
      ...
      if (isEdit) {
        axios
          .get(`https://api.kyuib.my.id/api/v1/categories/${idProject}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
          .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    const request = isEdit
      ? axios.put(`https://api.kyuib.my.id/api/v1/categories/${idProject}`, updatedFormData, config)
  ...
  const optionButton = (project, event) => {
            ...
            <div className="absolute flex justify-center items-center z-50 w-fit animate-fadeSlideRight" style={{ top: `${modalPosition.top}px`, left: `${modalPosition.left}px` }}>
              <div className="bg-white w-full max-w-sm sm:max-w-md md:max-w-3xl p-2 lg:p-4 lg:max-w-2xl rounded-2xl shadow-2xl space-y-4 overflow-y-auto max-h-screen relative text-sm text-black">
                <div className="">
                  ...
                  <div className="">
                    <button onClick={() => handleEditPage(selectedProject?.attributes?.slug)} className="bg-orange-400 hover:bg-orange-500 shadow-xl rounded-2xl h-[3rem] w-[3rem] flex items-center justify-center mb-2">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 text-white">
